...
def data_z(sheet, day, notarius, max_rows, max_cols):
    ...
    try:
        ...
        for dat in range(1, max_rows+1):
            ...
            if sheet.cell(row=dat, column=1).value is not None and day == sheet.cell(row=dat, column=1).value.strftime("%d.%m.%Y"):
                row_day = dat
...
#         save_file(sheet, time, day, power_of_attorney, name, last_name, tel, notarius)
...
def save_file(time, notarius, day, power_of_attorney, name, last_name, tel):
    ...
    try:
        for dat in range(1, max_rows+1):
            # ищем нужную дату в строке ат
            if worksheet.cell(row=dat, column=1).value is not None and\
                    day == worksheet.cell(row=dat, column=1).value.strftime("%d.%m.%Y"):
                row_day = dat
        for i in range(1, max_cols + 1):
                    ...
                    if worksheet.cell(row=i, column=j).comment:         # если комментарий есть
                        ...
                        if str(telephone) == tel:
                            ...
                            if time_zapis == "08:00:00":
                                data_zapis = str(worksheet.cell(row=i-1, column=1).value.strftime("%d.%m.%Y"))
                                print("data_zapis >>>>>>>>>>>>>>>>>>>>>>>>", data_zapis)
                            elif time_zapis == "09:00:00":
                                data_zapis = str(worksheet.cell(row=i-2, column=1).value.strftime("%d.%m.%Y"))
                                print("data_zapis >>>>>>>>>>>>>>>>>>>>>>>>", data_zapis)
                            elif time_zapis == "10:00:00":
                                data_zapis = str(worksheet.cell(row=i-3, column=1).value.strftime("%d.%m.%Y"))
                                print("data_zapis >>>>>>>>>>>>>>>>>>>>>>>>", data_zapis)
                            elif time_zapis == "11:00:00":
                                data_zapis = str(worksheet.cell(row=i-4, column=1).value.strftime("%d.%m.%Y"))
                                print("data_zapis >>>>>>>>>>>>>>>>>>>>>>>>", data_zapis)
                            elif time_zapis == "12:00:00":
                                data_zapis = str(worksheet.cell(row=i - 5, column=1).value.strftime("%d.%m.%Y"))
                                print("data_zapis >>>>>>>>>>>>>>>>>>>>>>>>", data_zapis)
                            elif time_zapis == "13:00:00":
                                data_zapis = str(worksheet.cell(row=i - 6, column=1).value.strftime("%d.%m.%Y"))
                                print("data_zapis >>>>>>>>>>>>>>>>>>>>>>>>", data_zapis)
                            elif time_zapis == "14:00:00":
                                data_zapis = str(worksheet.cell(row=i - 7, column=1).value.strftime("%d.%m.%Y"))
                                print("data_zapis >>>>>>>>>>>>>>>>>>>>>>>>", data_zapis)
                            elif time_zapis == "15:00:00":
                                data_zapis = str(worksheet.cell(row=i - 8, column=1).value.strftime("%d.%m.%Y"))
                                print("data_zapis >>>>>>>>>>>>>>>>>>>>>>>>", data_zapis)
                            elif time_zapis == "16:00:00":
                                data_zapis = str(worksheet.cell(row=i - 9, column=1).value.strftime("%d.%m.%Y"))
                                print("data_zapis >>>>>>>>>>>>>>>>>>>>>>>>", data_zapis)
                            elif time_zapis == "17:00:00":
                                data_zapis = str(worksheet.cell(row=i - 10, column=1).value.strftime("%d.%m.%Y"))
                                print("data_zapis >>>>>>>>>>>>>>>>>>>>>>>>", data_zapis)
                            elif time_zapis == "18:00:00":
                                data_zapis = str(worksheet.cell(row=i - 11, column=1).value.strftime("%d.%m.%Y"))
                                print("data_zapis >>>>>>>>>>>>>>>>>>>>>>>>", data_zapis)
                            if data_zapis.split(".")[2] >= DT.datetime.now().strftime("%d.%m.%Y").split(".")[2] and \
                                    data_zapis.split(".")[1] >= DT.datetime.now().strftime("%d.%m.%Y").split(".")[
                                1] and \
