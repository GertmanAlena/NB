...
def log_start(message, now_time):
    botlogfile = open('loggerBot.log', 'a', encoding='utf-8')
    print(f'{now_time.strftime("%d-%m-%Y %H:%M")} пользователь {message.from_user.first_name} id-{message.from_user.id} '
...
def log_error(db_connection_error):
    ...
    print(now_time.strftime("%d-%m-%Y %H:%M"), 'произошла ошибка при поиске совпадений '
                                    'по дате уведомления таблицы SQL ' % db_connection_error, file=botlogfile)
    botlogfile.close()
...
def person_in_db(message, l_name, tel):
    botlogfile = open('loggerBot.log', 'a', encoding='utf-8')
    print(f'{now_time.strftime("%d-%m-%Y %H:%M")} от пользователя {message.from_user.first_name} id-{message.from_user.id} из введённых данных сформированы l_name={l_name} и tel={tel}', file=botlogfile)
    botlogfile.close()
...
def person_add_bd(now_time, user_id, first_name, last_name, date, date_sms):
    botlogfile = open('loggerBot.log', 'a', encoding='utf-8')
    print(now_time.strftime("%d-%m-%Y %H:%M"), 'в базу добавлен пользователь ' + user_id, first_name, last_name, file=botlogfile)
    botlogfile.close()

def log_name_none(message, now_time):
    botlogfile = open('loggerBot.log', 'a', encoding='utf-8')
    print(now_time.strftime("%d-%m-%Y %H:%M"), 'у пользователя c id ' + {message.from_user.id} + 'имя не известно, поэтому занесено имя None',
          file=botlogfile)
    botlogfile.close()

def log_last_name_none(message, now_time):
    botlogfile = open('loggerBot.log', 'a', encoding='utf-8')
    print(now_time.strftime("%d-%m-%Y %H:%M"), 'у пользователя c id ' + {message.from_user.id} + 'фамилия не известна, поэтому занесено имя None',
          file=botlogfile)
    botlogfile.close()
